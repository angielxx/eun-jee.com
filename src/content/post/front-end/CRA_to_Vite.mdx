...
- 환경변수명 변경 : 기존에 사용하던 환경변수 REACT*APP*{variable name}에서 VITE\_{variable name}으로 변경됨
- 환경변수 호출방식 변경 : 기존 process.env.REACT*APP*{variable name}에서 import.meta.VITE\_{variable name} 으로 변경
...
// build-and-deploy-saas-fe-app.yml
  ...
  build-and-push:
    ...
    with:
      ...
      ECR_REPO_URL: 863067839821.dkr.ecr.ap-northeast-2.amazonaws.com
      ROLE_ARN: arn:aws:iam::863067839821:role/assume-role-surffy-dev
